...
function App() {
  ...
  const fetchData = async () => {
    const mainData = await fetch(
      "https://crossorigin.me/https://www.swiggy.com/dapi/restaurants/list/v5?lat=28.7040592&lng=77.10249019999999&collection=83633&tags=layout_CCS_NorthIndian&sortBy=&filters=&type=rcv2&offset=0&page_type=null"
    );
        ...
        <Grid2 size={{ xs: 12 }} className="card">
          <h2>
            ...
            <Button
              ...
              sx={{ marginLeft: "10px" }}
              onClick={() => {
                const filterData = cardData.filter((x) => x.avgRating > 4);
            ...
            <div className="search">
              ...
              <IconButton aria-label="search" onClick={()=>{
               const filterData =  cardData.filter((x)=> x.name.toLowerCase().includes(searchValue.toLowerCase()))
